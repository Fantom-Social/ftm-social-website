...
export default async function unfollow(user) {
    if (window.ethereum) { //check if Metamask is installed
        try {
            ...
            alert("You need to connect to FTM Social to unfollow someone.")
        }
    } else {
        alert("Please install metamask to your browser.")
    }
};
async function unfollowRequest(user) {
    ethereum.request({ method: 'eth_requestAccounts' }).then((accounts) => {
        web3.eth.estimateGas({
            to: contractAddress,
            from: ethereum.selectedAddress,
            data: contract.methods.unfollow(user).encodeABI(),
            value: 0
        }).then((data) => {
            ethereum
                .request({
